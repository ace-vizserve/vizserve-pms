...
| D25 | **Leave types live in an admin-editable table, not an enum.** Types are policy data that HR will change; request types and statuses are structural. An enum would need the two-migration dance every time, and could not retire a type without orphaning past requests. **Types are not balances** — balances stay out of scope (`no-leave-balance.test.ts` still guards it). The list itself is owed by Amier | User, 18 Aug 2026 |
