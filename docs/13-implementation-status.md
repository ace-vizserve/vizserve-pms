...
| **P8-13** | **The transport flips to Resend** — `RESEND_API_KEY` arrived, so `EMAIL_TRANSPORT=resend` and the inference order reverses (the switch itself is gone as of P8-16). Adapter-level pacing moved to `lib/email/transports/pacing.ts` so the switch did not leave the outbox drain unthrottled. The per-message reply-to the old EmailJS notes described was checked and deliberately **not** built | **Shipped; one real send still unproven** |
