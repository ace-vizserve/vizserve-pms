...
|---|---|---|
| **Email needs its EmailJS keys** | P8-10 made the transport a choice and pointed it at EmailJS, so `RESEND_API_KEY` being unset no longer silences the app. What it needs now is `EMAILJS_PRIVATE_KEY` **and** the **Account → Security** toggle that permits non-browser API requests — without either, `emailMode()` is `dry-run` and every send is still a logged no-op. This is what kept the Gate 3 client email silent for months while QA saw a clean pass. | Set `EMAILJS_PRIVATE_KEY`, enable non-browser API requests in the EmailJS dashboard, paste `docs/emailjs/template.html` into the template, and set `NEXT_PUBLIC_SITE_URL` so the approval link is not `localhost`. |
