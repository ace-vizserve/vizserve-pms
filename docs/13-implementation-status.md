...
|---|---|---|
| **Email is live and unproven** | P8-13 set `RESEND_API_KEY` and `EMAIL_TRANSPORT=resend`, and P8-15 pointed the four senders at the verified **vizserve.com** (D16/Q12), so the app is out of dry-run **for the first time** — `EMAILJS_PRIVATE_KEY` was never set, which means every send in this repo's history was a logged no-op. That is the risk: the code path from server action to inbox has never once run end to end against a real ESP, and it is now switched on for clients. | Send one: `EMAIL_TEST_RECIPIENT=you@… npm run email:test`. Confirm all four of `approvals@` / `notifications@` / `support@` / `survey@` are real monitored mailboxes (replies bounce otherwise), and that `NEXT_PUBLIC_SITE_URL` is not `localhost` or the approval button goes nowhere. |
...
- [x] A client receives an email, clicks, and approves without logging in *(verified end to end in dry-run; P8-13 switched the send on, and it now waits on a verified `EMAIL_FROM` domain)*
...
| **P8-13** | **The transport flips to Resend** — `RESEND_API_KEY` arrived, so `EMAIL_TRANSPORT=resend` and the inference order reverses (the switch itself is gone as of P8-16). Adapter-level pacing moved to `lib/email/transports/pacing.ts` so the switch did not leave the outbox drain unthrottled. The per-message reply-to the old EmailJS notes described was checked and deliberately **not** built | **Shipped; a real send is proven (11 messages, 21 Sep 2026)** |
| **P8-14** | **The email shell is restyled and EmailJS's template is deleted** — `lib/email/layout.ts` is now the app's own surface in table HTML — a white `Card` (grade, hairline border, `--elev-2`) on the grey ground, the `status-badge` chip with its dot, and a `--gradient-primary` button at the app's 40px control height. Only the outer proportions come from react-email's `01-Barebone` demo, borrowed as markup rather than as a dependency; the first cut followed it literally and sank a tinted panel INTO the card, which is the inward emboss the design system bans. `docs/emailjs/` and `tests/unit/emailjs-template.test.ts` are gone; `docs/email/` replaces them, with previews regenerated by `tests/email/preview.test.ts` on every test run. ⚠️ This left the adapter alive without its template; P8-16 finished the job and removed the adapter too | **Shipped; unreviewed in Outlook** |
| **P8-15** | **One sender becomes four** — `approvals@`, `notifications@`, `support@`, `survey@`, all on the verified vizserve.com. `sendEmail()` takes a required `sender`, so the compiler makes every new send choose one. Split by what the email IS, not who receives it, because a recipient mutes and marks-as-spam per sender — one address lets a client tired of surveys bin the Gate 3 email Phase 4 rests on. The EmailJS adapter could not have honoured it — its sender was fixed on the connected service — which was one more argument for P8-16 | **Shipped; no real send yet** |
...
- **Email goes through one port with two adapters** (P8-10). `sendEmail()` in `lib/email/send.ts` picks between `transports/emailjs.ts` and `transports/resend.ts`; `EMAIL_TRANSPORT` chooses, and an unconfigured transport degrades to dry-run rather than throwing. VizServe is on EmailJS — moving to Resend is one env var, because no call site names a transport. See *Outstanding* at the top for what is still unset.
