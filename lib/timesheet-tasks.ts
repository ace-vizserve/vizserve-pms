...
/**
 * The tasks a person may log time against — THE QUERIES, WITH NO CLIENT OF
 * THEIR OWN.
 *
 * ------------------------------------------------------------------------
 * ⚠️ P12-23 SPLIT THIS OUT OF `lib/timesheet-tasks-server.ts`, WHICH IS
 * `server-only`, AND THE SPLIT IS THE WHOLE POINT OF THE FILE. The picker now
 * reads its first page from the browser through `qk.loggableTasks()`, and a
 * `"use client"` module that reached the old file would have pulled
 * `@/utils/supabase/server` and `next/headers` into the browser bundle — which
 * typechecks, lints and tests clean and fails only in `npm run build`. Phase 4
 * split `lib/approvals-queue.ts` and `canAccessDepartmentScope` for exactly this
 * reason; this is the same move.
 *
 * ⚠️ AND IT IS A SPLIT RATHER THAN A COPY BECAUSE THE SCOPE RULE MUST HAVE ONE
 * HOME. The header below has said so since the day the page asked for
 * PIC-or-QA while `vizserve_pms_may_log_time` had allowed any assignee since
 * P7-13. Three callers now share this: the browser fetcher, the picker's search
 * action, and — through the thin wrapper next door — nothing else. A second copy
 * of the scoping is how that drift happens again.
 *
 * NOTHING ELSE CHANGED ON THE WAY. Every query, every filter and every comment
 * below is the one that was there; the only edit is that the Supabase client
 * arrives as a parameter instead of being built inside.
 * ------------------------------------------------------------------------
 *
 * ONE IMPLEMENTATION, THREE CALLERS: the timesheet page's initial list (now in
 * the browser), the picker's search action, and the server wrapper that keeps
 * the old call shape.
 *
...
/**
 * The half of the Supabase client these two queries touch.
 *
 * Structurally typed and `from`-only, so the SERVER client and the BROWSER
 * client both satisfy it and neither is named here — which is what stops this
 * file acquiring an import that would put it back on one side of the wire.
 */
export type LoggableTaskClient = Pick<SupabaseClient<Database>, "from">;
 ...
 */
export async function loadLoggableTaskLists(
  supabase: LoggableTaskClient,
  userId: string,
): Promise<string[]> {
  const [mine, added] = await Promise.all([
 ...
 *
 * The third caller of the scoping above: the empty rows on a week are ids in
 * `vizserve_pms_timesheet_layouts`. Same rule as the other two: `is_on_task` —
 * the accountable name, the QA reviewer, or a row in
 ...
 * disappears — they could not log against it anyway. The id stays in the table
 * so the row returns if they are put back on it.
 *
 * ⚠️ `id.in.(…)` IS THE ONE PLACE THIS FILE ALLOWS IT. The header's warning is
 * about an UNBOUNDED list; this one is capped at `MAX_LAYOUT_ROWS` by the schema
 * on the way in, by a CHECK constraint in the table, and sliced again below.
 *
 * ORDER IS THE CALLER'S — `orderRows` in lib/timesheet-row-order.ts.
 */
export async function loadLoggableTasksByIds(
  client: LoggableTaskClient,
  userId: string,
  ids: string[],
  ...
  // The same two-request shape `loadLoggableTasks` uses: PostgREST cannot
  // express "or exists in that other table".
  const [mine, added] = await Promise.all([
    client
      .from("vizserve_pms_tasks")
      .select(TASK_COLUMNS)
  ...
  // PIC and a join-table row is ordinary; the same task twice would be the same
  // ROW twice, writing into one cell.
