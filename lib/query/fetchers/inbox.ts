...
import type { Database, VizservePmsNotificationType } from "@/lib/database.types";
 ...
 * column outright.
 */
/** The narrowest client a fetcher needs; the browser and server clients both fit. */
export type InboxReadClient = Pick<SupabaseClient<Database>, "from">;
 ...
 */
export async function fetchUnreadCount(client: InboxReadClient): Promise<number> {
  return readCount(
    client
      .from("vizserve_pms_notifications")
