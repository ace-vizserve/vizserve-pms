...
import type { Database, VizservePmsTaskStatus } from "@/lib/database.types";
import { addDays, weekDates, workedMinutes } from "@/lib/dates";
...
export async function fetchTeamWeek(
  ...
  const [
    ...
    read<unknown[]>(
      client
        ...
        // Same order as `lib/timesheet-entries-server.ts`, which main's RSC used.
        .order("work_date")
        .order("created_at"),
    ),
   ...
   * The calendar returns requested leave too, so the team calendar can show what
   * has been asked for. This grid marks a week "on leave" so a lead does NOT
   * chase somebody for a timesheet — and a request nobody has decided excuses
   * nothing, least of all in advance of the decision that might refuse it.
   */
...
/* -------------------------------------------------------------------------- */
/* `qk.weekLayout(userId, weekStart)` — the shortcut and the saved layout.     */
/* -------------------------------------------------------------------------- */

/** A task offered as a row: what `PickableTask` in `week-grid.tsx` needs. */
export type LayoutTask = Pick<LoggableTask, "id" | "title" | "where"> &
  Partial<Pick<LoggableTask, "start_date" | "due_date">> & { status: VizservePmsTaskStatus };

export type WeekLayout = {
  /** P6-02b — last week's tasks, once each, for "Last week's tasks". */
  previousWeekTasks: LayoutTask[];
  /** P6-02d — the week's saved arrangement. Seeds `useLayoutAutosave`. */
  layout: { extraTasks: LayoutTask[]; rowOrder: string[]; alreadyCopied: boolean };
};

const layoutRowSchema = z.object({
  extra_task_ids: z.array(z.string()),
  row_order: z.array(z.string()),
  copied_last_week: z.boolean(),
});

const lastWeekEntrySchema = z.object({
  task_id: z.string(),
...
/**
 * P6-02b/P6-02d, ported from `main`'s RSC.
 *
 * ⚠️ LAST WEEK IS THE TASKS, NOT THE HOURS. The shortcut adds empty ROWS — the
 * same thing "Add task" adds — so nothing reaches the database until somebody
 * types a duration. A task this person can no longer read comes back with a
 * null embed (the LEFT embed that must never become `!inner`) and is DROPPED:
 * `vizserve_pms_may_log_time` would refuse every keystroke on it.
 *
 * ⚠️ THE SHORTCUT FAILING COSTS THE SHORTCUT AND NOTHING ELSE — no banner, a
 * missing shortcut states nothing false. The LAYOUT failing does throw: seeding
 * the autosave from an empty layout would save that emptiness back over the
 * real one on the first drag.
 *
 * The layout's ids need a second round trip — they come out of the first.
 */
export async function fetchWeekLayout(
  client: TimesheetReadClient,
  params: { userId: string; monday: string },
): Promise<WeekLayout> {
  const { userId, monday } = params;
  ...
  const [lastWeek, layoutRow, departmentRows, listRows] = await Promise.all([
    tolerate(
      read<unknown[]>(
        client
          .from("vizserve_pms_timesheet_entries")
  ...
  const layout = layoutRow === null ? null : parse(layoutRowSchema, layoutRow, "this week's layout");
  const departmentName = new Map(
    parseAll(namedRowSchema, departmentRows, "departments").map((row) => [row.id, row.name]),
  );
  const listName = new Map(
    parseAll(namedRowSchema, listRows, "lists").map((row) => [row.id, row.name]),
  );

  // "Department / List" — the same line the grid draws under a task name.
  const whereOf = (task: { department_id: string | null; list_id: string | null }) =>
    [
      task.department_id ? departmentName.get(task.department_id) : null,
  ...
  const lastWeekEntries =
    lastWeek.data === null ? [] : parseAll(lastWeekEntrySchema, lastWeek.data, "last week's tasks");

  const previousWeekTasks = [
    ...new Map(
      lastWeekEntries.flatMap((entry) => {
        ...
        return [
          [
            entry.task_id,
            {
              id: entry.task_id,
              title: task.title,
              status: task.status as LayoutTask["status"],
              where: whereOf(task),
            },
          ] as const,
        ];
  ...
  const extraTasks: LayoutTask[] = layout
    ? (await loadLoggableTasksByIds(client, userId, layout.extra_task_ids)).map((task) => ({
        id: task.id,
        title: task.title,
  ...
  return {
    ...
    layout: {
      extraTasks,
      rowOrder: layout?.row_order ?? [],
      alreadyCopied: layout?.copied_last_week ?? false,
    },
  };
}
