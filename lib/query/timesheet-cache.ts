...
/**
 * P7-05b — a withdrawn submission is a DRAFT again: the row is deleted, so the
 * week reads `null`, exactly as a week nobody has submitted.
 */
export function patchWeekWithdrawn(client: QueryClient, weekKey: QueryKey): void {
  client.setQueryData(weekKey, (data: unknown) => {
    if (!isRecord(data)) return data;
