import { isTaskImageSrc, MENTION_ATTR, RICH_TEXT_TAGS, richTextToPlainText } from "@/lib/rich-text";
...
/** The same test as `MENTION_ID` in `lib/rich-text-server.ts`. */
...
function walk(source: Node, target: Element, doc: Document): void {
  for (const child of Array.from(source.childNodes)) {
     ...
     */
    /*
     * P7-67 — AN IMAGE IS KEPT ONLY WHEN IT POINTS AT A TASK IMAGE, and then
     * dropped whole otherwise — `exclusiveFilter` in `sanitizeRichText`. An `img`
     * with an arbitrary `src` is a tracking pixel in a colleague's comment.
     */
    if (tag === "img" && !isTaskImageSrc(element.getAttribute("src"))) continue;

    const safe = doc.createElement(tag);

    if (tag === "img") {
      // Mirrors the `img` transform in `lib/rich-text-server.ts`, value for value.
      safe.setAttribute("src", element.getAttribute("src")!);
      ...
      for (const dimension of ["width", "height"] as const) {
        ...
        if (value && /^[0-9]{1,5}$/.test(value)) safe.setAttribute(dimension, value);
      }

      const orientation = element.getAttribute("data-orientation") ?? "";
      if (["landscape", "portrait", "square"].includes(orientation)) {
        safe.setAttribute("data-orientation", orientation);
      }
    }

    /*
     * P7-71 — a `span` carries a mention's user id and NOTHING ELSE: no `class`,
     * no `style`. A malformed id is dropped and the text stays.
     */
    if (tag === "span") {
      const id = element.getAttribute(MENTION_ATTR) ?? "";
      if (MENTION_ID.test(id)) safe.setAttribute(MENTION_ATTR, id);
    }
