...
describe("sanitizeRichTextInBrowser — the guard", () => {
  ...
  it("unwraps a disallowed wrapper rather than eating its prose", () => {
    ...
    // the server keeps the children. A wrapper around a sentence must not take
    // the sentence with it. `DROP_WITH_CONTENT` is the short list of exceptions.
    // (`span` itself is allowed since P7-71, so the wrapper here is `section`.)
    const out = sanitizeRichTextInBrowser("<div><section>kept</section></div>");
...
describe("sanitizeRichTextInBrowser — images and mentions, as the server has them", () => {
  const id = "0f8fad5b-d9cb-469f-a165-70867728950e";

  it("keeps a task image with its measured size and drops everything else on it", () => {
    const out = sanitizeRichTextInBrowser(
      `<p><img src="/api/task-images/${id}" alt="shot" width="640" height="abc" data-orientation="portrait" style="position:fixed" onerror="alert(1)"></p>`,
    );
    expect(out).toContain(`src="/api/task-images/${id}"`);
    expect(out).toContain('alt="shot"');
    expect(out).toContain('width="640"');
    expect(out).toContain('data-orientation="portrait"');
  ...
  it("drops an image pointing anywhere else, whole", () => {
    const out = sanitizeRichTextInBrowser('<p>a<img src="https://evil.example/p.gif">b</p>');
    expect(out).not.toContain("img");
    expect(out).toContain("ab");
  });

  it("keeps a mention's id and nothing else on the span", () => {
    const out = sanitizeRichTextInBrowser(
      `<p><span data-mention-id="${id}" class="bg-red-500" style="x">@Ace</span></p>`,
    );
    expect(out).toContain(`data-mention-id="${id}"`);
    expect(out).not.toContain("class");
    expect(out).not.toContain("style");
  });

  it("drops a malformed mention id but keeps the text", () => {
    const out = sanitizeRichTextInBrowser('<p><span data-mention-id="nope">@Ace</span></p>');
