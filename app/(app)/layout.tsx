      ...
      <BreadcrumbLabelProvider>
        ...
        */}
        ...
        {/*
          ...
          authenticated page, and the combined burst started failing with
          `TypeError: fetch failed`. The component fetches its own state after
          mount; see `app/(app)/reminder-actions.ts`.

          P12-23: `viewerId` costs no query — `context` is already in hand. It
          names `qk.punchState()`, the SAME cache entry the punch panel reads on
          `/`, `/dashboard` and `/dtr`, so on those pages the reminder is free.
        */}
