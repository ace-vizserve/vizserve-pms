...
import {
  ...
  fetchTimesheetWeek,
  fetchWeekLayout,
} from "@/lib/query/fetchers/timesheet";
...
export function TimesheetView({
  ...
  if (layoutQuery.isError) {
    return <QueryError what="this week's layout" message={layoutQuery.error.message} />;
  }

  // The grid waits for the layout: it seeds state on the first render only.
  if (weekQuery.isPending || layoutQuery.isPending) return <WeekGridSkeleton />;
    ...
    <>
      ...
      */}
      ...
      <WeekGrid
        ...
        // P6-02b. Offered as empty rows, never as hours.
        previousWeekTasks={layoutQuery.data.previousWeekTasks}
        // P6-02d. Seed values — `useLayoutAutosave` owns the layout from the
        // first render on. The `key` re-seeds it on a week change, and makes a
        // pending debounce flush against the week it was editing.
