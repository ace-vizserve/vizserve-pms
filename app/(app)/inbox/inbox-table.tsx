 ...
 */
const INBOX_ROOTS = [["inbox"], qk.unread(), qk.snapshot()] as const;
...
function patchRead(client: QueryClient, matches: (row: Notification) => boolean): void {
  ...
  /*
   * The rail's red badge, the same guess on the same terms — it is a field of
   * the sidebar snapshot, so without this the sidebar would sit one higher than
   * the row just clicked until `onSettled`'s refetch lands.
   */
  client.setQueryData<SidebarSnapshot>(qk.snapshot(), (current) =>
    current === undefined ? current : { ...current, unread: Math.max(0, current.unread - marked) },
  );
}
