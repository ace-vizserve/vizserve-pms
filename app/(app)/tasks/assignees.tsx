...
/**
 * THE SAME PICKER, BEFORE THE TASK EXISTS.
 *
 * `AssigneePicker` above writes on every click, because it edits a task that is
 * already there. On a CREATION form there is nothing to write to yet, so this
 * one is controlled: it holds a list of ids, and the action adds them all after
 * the row is made. Two components rather than a `mode` prop, because "commit
 * immediately" and "collect a draft" share their looks and nothing else — the
 * pending state, the toasts and the router refresh above are all meaningless
 * here.
 *
 * ⚠️ IT DOES NOT OFFER THE OWNER, AND IT DOES NOT NEED TO. `assignee_id` is set
 * by the form's own person-in-charge control and the create function puts that
 * person on the join table itself. The caller filters them out of `candidates`;
 * `insertTask` drops them again server-side, because a UI sending the same
 * person in both fields is a UI behaving normally and not an error to report.
 *
 * The list here is a PROPOSAL. `vizserve_pms_add_task_assignee` is the only way
 * into the join table and it re-checks that each person is an active member of
 * the task's department, so a stale name comes back as a sentence rather than
...
export function PeoplePicker({
  ...
  onChange,
  disabled,
  placeholder = "Also working on it",
  triggerClassName,
  align = "start",
}: {
  value: string[];
  /** Who may be added. The caller excludes the person in charge. */
  candidates: Person[];
  onChange: (next: string[]) => void;
  disabled?: boolean;
  /** What the trigger says when nobody is chosen. */
  placeholder?: string;
  /** The surface's own trigger shape — a form field, or a composer chip. */
  ...
  const chosen = useMemo(
    () => candidates.filter((person) => value.includes(person.id)),
    [candidates, value],
  );
  ...
  const shown = useMemo(() => {
    ...
    return candidates.filter((person) => person.full_name.toLowerCase().includes(needle));
  }, [candidates, query]);

  function toggle(id: string) {
    onChange(value.includes(id) ? value.filter((each) => each !== id) : [...value, id]);
  }
  ...
  return (
    ...
    >
      <PopoverTrigger
        ...
        aria-label={
          chosen.length
            ? `Also working on it: ${chosen.map((person) => person.full_name).join(", ")}. Change it.`
            : "Add other people to this task"
        }
        className={cn(
          triggerClassName,
          chosen.length ? "border-accent-border bg-accent" : "border-dashed",
        )}
      >
        {chosen.length === 0 ? (
          <>
          ...
          <>
            <span className="inline-flex shrink-0 items-center">
              {chosen.slice(0, 3).map((person, index) => (
                <Monogram
                  key={person.id}
                  id={person.id}
                  name={person.full_name}
                  // Overlapped, with a surface ring so two tiles never read as
                  // one shape.
                  className={cn("size-5 text-[10px]", index > 0 && "-ml-1.5 ring-2 ring-card")}
                />
              ))}
            </span>
            <span className="truncate">
              {/* The count is spelled out rather than left to the monograms:
                  three initials and a "+2" is a picture, not a label. */}
              {chosen.length === 1 ? chosen[0].full_name : `${chosen.length} people`}
            </span>
          </>
        )}
      </PopoverTrigger>
      ...
      <PopoverContent align={align} className="w-64 p-0">
        ...
        <div className="max-h-72 overflow-y-auto py-1">
          {candidates.length === 0 ? (
            <p className="px-3 py-1.5 text-2xs text-muted-foreground">
              Nobody else in this department.
            </p>
          ) : shown.length === 0 ? (
            ...
            shown.map((person) => {
              ...
              return (
                <button
                  key={person.id}
                  type="button"
                  // ⚠️ Announced as a checkbox, because that is what it is: the
                  // popup stays open and the row toggles. A plain button would
                  // be read as "activates something" and give no state at all,
                  // which is the half the tick mark cannot carry for a screen
                  // reader.
                  role="checkbox"
                  aria-checked={picked}
                ...
                >
                  ...
                  <span className="min-w-0 flex-1 truncate text-left">{person.full_name}</span>
                  <Check
                    className={cn("size-3.5 shrink-0 text-primary", !picked && "opacity-0")}
                    aria-hidden
                  />
                </button>
              );
