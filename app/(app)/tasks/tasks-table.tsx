...
import type { VizservePmsTaskStatus, VizservePmsUserRole } from "@/lib/database.types";
...
import { TaskSelectCheckbox } from "./task-selection";
 ...
 *
 * WARNING - NO `urlSort` HERE, AND THAT IS DELIBERATE. The list is grouped by
 * stage and every group renders its own `bare` table, so a column sort would
 * reorder eight tables independently and mean nothing across them. Ordering
 * stays with the existing `?sort=` control in `filters.tsx`, which sorts the
 * query before the rows are grouped.
 */

/** A Map's `.get` over the plain object that replaced it. */
function get<T>(record: Record<string, T>, key: string | null | undefined): T | undefined {
 ...
 */
export type ListRow = TaskRow & { depth: 0 | 1 };
...
export function TaskGroupTable({
  ...
  function canDelete(task: TaskRow) {
    ...
    const leads =
      viewer.role === "admin" || viewer.managedDepartmentIds.includes(task.department_id);
    return (
      leads ||
  ...
  function seat(task: TaskRow) {
    return {
      ...
      leadsDepartment: viewer.role === "admin" || viewer.managedDepartmentIds.includes(task.department_id),
      isAdmin,
    };
  }
  ...
  const columns: Column<ListRow>[] = [
        ...
        return (
          ...
          <div className={cn("group/task", isChild && "pl-6")}>
            ...
            <div className="mt-0.5 flex flex-wrap items-center gap-x-2 text-2xs text-muted-foreground">
              {task.list_id ? <span>{get(lookups.listName, task.list_id)}</span> : null}
    ...
    },
    {
      ...
      cell: (task) => (
        <AssigneePicker
          taskId={task.id}
          pic={task.assignee_id ? { id: task.assignee_id, full_name: get(lookups.nameOf, task.assignee_id) ?? "—" } : null}
          ...
          others={(get(lookups.extraAssignees, task.id) ?? []).filter((person) => person.id !== task.assignee_id)}
    ...
    },
    {
      ...
      cell: (task) => {
        const closed = isTerminal(task.status) ? get(lookups.closedOn, task.id) : null;
