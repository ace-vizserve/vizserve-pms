-- P7-77 — who hears about a client request at Gate 1.
--
-- Settled 23 Sep 2026:
--
--   ON SUBMISSION   the client (acknowledgement, sent from the server action)
--                   and the department's TEAM LEADERS.
--   ON APPROVAL     the client (sent from the server action), the PIC, the QA
--                   reviewer, and the department's TEAM LEADERS.
--
-- Managers, admins and owners who also manage the department were getting the
-- "Approval needed" email too, because the submission notified every managed-
-- department user with `role >= 'team_leader'`. They still get the INBOX row —
-- they can still approve, and a queue they cannot see is worse than an email
-- they did not want — but not the email.
--
-- ⚠️ `role = 'team_leader'`, NOT `>=`. The one place in this repo where the
-- equality is deliberate: this is about who is emailed, not who is allowed.
-- Authorization still reads `>=` everywhere.
--
-- ALSO: the PIC and the QA reviewer are written into `vizserve_pms_task_assignees`
-- on approval, so the task's assignee list shows both of them from the start.
--
-- BOTH FUNCTIONS ARE REPRODUCED WHOLE, copied from the migrations that last
...
--
-- ⚠️ APPLY 20260923090000 FIRST — it adds the enum value this file uses.
-- ---------------------------------------------------------------------------

insert into vizserve_pms_notification_type_settings (type, send_email, description) values
  ('request_approved', true,
   'P7-77. A client request in a department you lead was approved at Gate 1. Team Leaders only.')
...
begin
  ...
  -- P7-77. The department's Team Leaders — `=`, not `>=`, see the header.
...
begin
  ...
  -- P7-77. Everyone above gets the inbox row; only Team Leaders get the email.
  -- Managers, admins and owners can still approve, so they keep the row.
  update vizserve_pms_notifications n
     set send_email = false
    from vizserve_pms_users u
   where n.user_id = u.id
     and n.entity_type = 'request'
     and n.entity_id = v_request_id
     and n.type = 'pending_approval'
     and u.role <> 'team_leader';
