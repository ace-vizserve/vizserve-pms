-- ---------------------------------------------------------------------------
-- P12-04 — the task SELECT policy stops being evaluated once per row.
--
-- THE BUG. The sidebar reports "Couldn't load your lists" and the browser
-- console says `canceling statement due to statement timeout` (57014), over
-- 3,926 tasks. `sidebar-panel.tsx` ends every read in `?? []`, so a 57014
-- renders as "zero open tasks" and a refresh appears to fix it. That is the
-- bug Amier originally reported, and it has been mis-attributed to a request
-- burst since.
--
...
-- are in each list".
--
-- THE FIX: wrap every SESSION-CONSTANT subexpression in a scalar subquery.
-- `(select f())` is evaluated once as an InitPlan and the result reused for
-- every row; `f()` is evaluated per row. This is the standard Supabase RLS
-- remedy and it changes nothing about the value computed.
--
-- ⚠️ THIS CHANGES HOW THE POLICY IS EVALUATED. IT DOES NOT CHANGE WHO CAN SEE
-- WHAT. Every clause below admits the same rows as the P11-08 version. If you
-- are reviewing this and cannot convince yourself of that for a given line,
-- STOP — a silent widening here hands one department's work to another, and
-- the failure mode is that everything looks fine.
--
-- ---------------------------------------------------------------------------
-- ⚠️ TWO TRAPS, RECORDED FROM AN EARLIER ATTEMPT AT THIS SAME CHANGE.
--
-- That attempt (`P12-03`, on the `staging` branch) is deliberately NOT part of
-- this migration set and must never be applied to this project: it broke every
-- task read. It is documented here rather than shipped because both traps are
-- live constraints on anyone editing the policy below.
--
-- TRAP 1 — RECURSION.
...
-- would evaluate the join table's RLS once per row." That attempt did it from
-- inside the policy itself.
--
-- TRAP 2 — A DROPPED BRANCH. That attempt quoted a two-clause
...
--
-- WHAT THIS MIGRATION DOES, THEN. Every session-constant
...
comment on function vizserve_pms_my_task_ids() is
  ...
  'recursion a bare subquery here would introduce. Parameterless and filtered on auth.uid(), so it '
...
-- reliever branch stated explicitly as its own disjunct.
-- ---------------------------------------------------------------------------
...
create policy "tasks readable by participants and department leads"
  ...
  using (
    ...
    /*
     * P9-01's reliever branch, STATED EXPLICITLY. It lives inside
     * `vizserve_pms_is_on_task`, which this policy no longer calls — and every
     * published body of that function predates P9-01 adding it, which is how
     * TRAP 2 above lost it silently.
     *
