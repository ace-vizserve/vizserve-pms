...
- **A task belongs to its department, not to its PIC (P11-03, 7 Sep 2026).** Any active member of a task's department may edit it, log time against it, and rename a list — reversing P7-14. `status` is NOT included (outside the column grant; only `vizserve_pms_transition_task` moves it), and neither is deleting a task or creating a list. DTR, timesheet and internal approvals are untouched. The permission was granted on condition that everything is audited, which is why `vizserve_pms_audit_row_update` exists — before it, this app had **no audit triggers at all** and a direct `UPDATE` left no trace.
