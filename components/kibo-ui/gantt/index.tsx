...
/*
 * VENDORED from the Kibo UI shadcn registry (https://www.kibo-ui.com/r/gantt.json).
 *
 * ⚠️ LOCAL MODIFICATIONS — re-pulling this file from the registry DISCARDS them:
 *   1. `ContextMenuTrigger` takes Base UI's `render` prop instead of Radix's
 *      `asChild`; this app's shadcn style is base-nova and has no Radix.
 *   2. `GanttFeatureItemCard` accepts `fill`/`ink` and renders a PLAIN DIV
 *      rather than `<Card>`, so a bar is actually filled with its stage colour.
 *      Upstream the card is `bg-background` and the only tinted thing is the
 *      8px dot in the sidebar — every bar is the same neutral slab. `<Card>`
 *      cannot be tinted at all here: it carries `grade-surface`, an opaque
 *      white `background-image`, which paints over any `backgroundColor`.
 *
 * Its dependencies (date-fns, jotai, lodash.throttle, @uidotdev/usehooks) are
 * fenced to this folder by `no-restricted-imports` in eslint.config.mjs.
 */
...
export type GanttStatus = {
  id: string;
  name: string;
  /** Bar fill. A CSS colour — this app passes a `var(--gantt-bar-*)`. */
  color: string;
  /** LOCAL: label colour on that fill. Falls back to the card's own colour. */
  ink?: string;
};
...
export type GanttFeatureItemCardProps = Pick<GanttFeature, "id"> & {
  children?: ReactNode;
  /** LOCAL: bar fill, and the label colour measured against it. */
  fill?: string;
  ink?: string;
};
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    // LOCAL: A PLAIN DIV, NOT <Card>, and that is the whole fix for bars that
    // rendered white with a coloured edge. `Card` carries `grade-surface` —
    // `background-image: linear-gradient(180deg,#ffffff,#fafcfd)`, an OPAQUE
    // white gradient that paints straight over `backgroundColor` however it is
    // set. It also brings `py-4`, `gap-4` and `shadow-raised-lg`: panel
    // furniture on a 32px bar.
    //
    // The fill identifies nothing on its own (it sits ~1.2:1 against the page);
    // the 1px `borderColor` is what makes the bar an object, and the label
    // wears the same colour.
    <div
      className="h-full w-full overflow-hidden rounded-md border bg-card px-2 text-xs"
      style={fill ? { backgroundColor: fill, borderColor: ink, color: ink } : undefined}
    >
      <div
        className={cn(
...
> = memo(({ label, date, id, onRemove, className }) => {
  ...
  return (
    ...
    >
      <ContextMenu>
        {/* Base UI takes a `render` prop where Radix used `asChild`. */}
        <ContextMenuTrigger
          render={
            <div
              className={cn(
